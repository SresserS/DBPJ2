...
///////////4 functions in all,when use, make sure only one same-name function appears!!!!!!!!!!!!
...
Datum levenshtein_distance(PG_FUNCTION_ARGS)
{
    ...
    if(!is_inited){
        ...
        for(int i = 0; i <= 100; i++){//divide the two loop, use the Locality,reduce load cost
            dis[0][i]=i;
        }
        is_inited = true;
    }
	...
	for (int j = 1; j <= n; j++)
		for (int i = 1; i <= m; i++){
            if(tolower(s[i-1]) == tolower(t[j-1])){
                min = (dis[i][j-1] < dis[i-1][j-1])? dis[i][j-1]+1: dis[i-1][j-1];
                dis[i][j] = (dis[i-1][j]+1 < min)? dis[i-1][j] + 1 : min;
	}
            else{
                min = (dis[i][j-1] < dis[i-1][j-1])? dis[i][j-1] : dis[i-1][j-1];
                dis[i][j] = (dis[i-1][j] < min)? dis[i-1][j] + 1 : min + 1;
...
Datum levenshtein_distance(PG_FUNCTION_ARGS)
{
    ...
    static int dis[110][110];
    static bool is_inited = false;
    static int lastm = 0, lastn = 0;
    static char lasts[110],lastt[110];
    int samepos_s = 0, samepos_t = 0;
    ...
    for(int i = 0; i < min(lastm,m); i++){
        if(tolower(s[i])==tolower(lasts[i])) samepos_s++;
        else break;
    }
    for(int i = 0; i < min(lastn,n); i++){
        if(tolower(t[i])==tolower(lastt[i])) samepos_t++;
        else break;
    }
	...
	for (int i = samepos_s + 1; i <= m; i++)
		for (int j = 1; j <= samepos_t; j++){
            if(tolower(s[i-1]) == tolower(t[j-1]))
                dis[i][j] = dis[i-1][j-1];
            else{
    ...
    for (int i = 1; i <= samepos_s; i++)
		for (int j = samepos_t + 1; j <= n; j++){
            if(tolower(s[i-1]) == tolower(t[j-1]))
                dis[i][j] = dis[i-1][j-1];
            else{
                dis[i][j] = min(dis[i-1][j] + 1, min(dis[i][j-1] + 1, dis[i-1][j-1] + 1));
...
Datum jaccard_index (PG_FUNCTION_ARGS)
{
    ...
    static int table[16390];//2`14=16384
    ...
    if(m == 0 || n == 0) result = 0;
        ...
        int a = 36<<7;
        int catascii = a + tolower(s[0]);
        table[catascii] = 1;
        visit[sumofunion] = catascii;  
        sumofunion++;
        for(int i = 1; i < m; i++){
            //catascii = s[i]%128;
            catascii = catascii % 128;
            a = catascii<<7;
            catascii = a + tolower(s[i]);
            if(table[catascii] == 0){
                visit[sumofunion] = catascii;    
                sumofunion++;
                table[catascii] = 1;
        ...
        for(int i = 1; i < n; i++){
            catascii = catascii % 128;
            a = catascii<<7;
            catascii = a + tolower(t[i]);
            if(table[catascii] == 0){
                visit[sumofunion] = catascii;  
                sumofunion++;
                table[catascii] = -1;
            }
            else if(table[catascii] == 1){
                sumofintersection++;
                table[catascii] = -1;
            }
        }
        ...
        result = (float4)sumofintersection/sumofunion;
    } 
    for(int i = 0;i < sumofunion; i++) table[visit[i]] = 0;
...
Datum jaccard_index (PG_FUNCTION_ARGS)
{
    ...
    static int table[129][129];//2`14=16384
    int sumofintersection = 0;
    int sumofunion = 0;
    int visit[2][1000];
    ...
    if(m == 0 || n == 0) result = 0;
        ...
        table['$'][s[0]] = 1;
        visit[0][sumofunion] = '$';
        visit[1][sumofunion] = s[0];
        sumofunion++;
        
        for(int i = 0; i < m - 1 ; i++){
            if(table[s[i]][s[i+1]] == 0){
                visit[0][sumofunion] = s[i];
                visit[1][sumofunion] = s[i+1];
                sumofunion++;
                table[s[i]][s[i+1]] = 1;
        ...
        if(table['$'][t[0]] == 1){
            ...
            visit[0][sumofunion] = '$';
            visit[1][sumofunion] = t[0];
            sumofunion++;
        }

        for(int i = 0; i < n-1; i++){
            if(table[t[i]][t[i+1]] == 0){
                visit[0][sumofunion] = t[i];
                visit[1][sumofunion] = t[i+1];
                sumofunion++;
                table[t[i]][t[i+1]] = -1;
            }
            else if(table[t[i]][t[i+1]] == 1){
                sumofintersection++;
                table[t[i]][t[i+1]] = -1;
            }
    ...
    for(int i = 0; i < sumofunion; i++){
        table[visit[0][i]][visit[1][i]] = 0;
    }
